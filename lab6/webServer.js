...
app.get("/user/list", (request, response) => {
    ...
    User.find({}, (err, users) => {
        ...
        response.status(200).end(JSON.stringify(userList));
    });
});
    ...
    User.find({}, (err, data) => {
        if (err) {
            ...
            if(user[0]!=undefined){
                ...
                response.status(200).end(JSON.stringify(userDetail));
                return;
            }else{
                response.status(400).end('err');
                return;
            ...
            return;
                ...
                Promise.all(data.map( async  el => {
                    return {
                        ...
                        comments: await Promise.all(el.comments.map(async com => {
                            return {
                                comment: com.comment,
                                date_time: com.date_time,
                                user: await promise(com.user_id)
                            };
                        }))
                    };
                })).then(userPhotos=>{
